...
from numpy import fromfile,prod,sqrt,ndarray,load
...
class Params(object):
    ...
    def __init__(self,
        ...
        if isinstance(params,file):
            ...
            if dtype is None:
                dtype = float
            self.params = load(params) #[fromfile(params,count=prod(s),dtype=dtype).reshape(s) for s in shapes]
        ...
        # If params is a string, try opening that file and loading params from it
            ...
            with open(params,'rb') as f:
                self.params = load(f) #.reshape(s) for s in shapes
