...
from numpy import spacing,prod,exp,log,inf,isfinite,load,fromfile
import numpy as np
...
class DecayRate(LearningRate):
    ...
    def __init__(self,
                 ...
                 initialRate = 1e-13, # Initial learning rate
...
"""
def gradDescent(prefix,spikes,stim,jack,fsize,extrapSteps,
                pixelNorm,filepath,model,
                maxIts,maxHours,perm,overwrite,
                Njack,start,nlags,splits,
                LRType,LRParams):
...
#    del spikes
    ...
    spikesmean = np.nanmean(YR)
    Nspikes = np.nansum(YR)
    ...
    stdout.write('Validation: {0} frames, {1} spikes\n'.format(Nvalid,np.nansum(YV)))
    stdout.flush()

    # Create filenames
    trainBestName = filepath+prefix+AlgTag+'_train_%u.npy' % (jack,)
    validBestName = filepath+prefix+AlgTag+'_valid_%u.npy' % (jack,)
    ...
    if exists(statusName):
        ...
        if exists(trainBestName) and not overwrite:
            ...
            np.save(trainBestName, P) #dtype=str)
            np.save(validBestName, PV) # dtype=str)
